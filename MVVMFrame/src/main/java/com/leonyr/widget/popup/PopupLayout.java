package com.leonyr.widget.popup;
...
public class PopupLayout {
     ...
     * @param contentLayoutId 内容布局Id
     */
    public static PopupLayout init(Context context,@LayoutRes int contentLayoutId, @DrawableRes int backgroundRes){
        PopupLayout popupLayout=new PopupLayout();
        popupLayout.mPopupDialog=new PopupDialog(context);
        popupLayout.mPopupDialog.setBackgroundDraw(backgroundRes);
        popupLayout.mPopupDialog.setContentLayout(contentLayoutId);
        popupLayout.initListener();
        return popupLayout;
    }
    ...
    //初始化Dialog监听器
    private PopupLayout initListener(){
    ...
    public PopupLayout setBackgroundDraw(@DrawableRes int draw){
        if (mPopupDialog != null){
            mPopupDialog.setBackgroundDraw(draw);
        }
        return this;
    }
     ...
     * 设置是否使用圆角特性
     */
    public PopupLayout setUseRadius(boolean useRadius){
        if(mPopupDialog!=null){
     ...
     */
    public PopupLayout setHeight(int height,boolean dpMode){
        if(dpMode){
     ...
     * @param dpMode 是否以dp为单位
     */
    public PopupLayout setWidth(int width,boolean dpMode){
        if(dpMode){
