...
public class HeartLikeView extends RelativeLayout {
    ...
    private final String TAG = HeartLikeView.class.getName();
    ...
    public HeartLikeView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray typedArray = getContext().obtainStyledAttributes(attrs, R.styleable.HeartLikeView);
        // 默认图片 根据他获取飘心图的宽高 *重要
        int defaultFavor = typedArray.getResourceId(R.styleable.HeartLikeView_ksg_default_image, -1);
        // 进入动画时长
        int enterDuration = typedArray.getInteger(R.styleable.HeartLikeView_ksg_enter_duration, 1500);
        // 曲线动画时长
        int curveDuration = typedArray.getInteger(R.styleable.HeartLikeView_ksg_curve_duration, 4500);
    ...
    private void init(int defaultFavor, int enterDuration, int curveDuration) {
        ...
        // 初始化飘心动画路径
        this.mPathAnimator = new HeartPathAnimator(enterDuration, curveDuration);
